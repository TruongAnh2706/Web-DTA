...
const url = "https://raw.githubusercontent.com/duyet/vietnamese-wordlist/master/vietnamese-wordlist.txt";
...
https.get(url, (res) => {
  ...
  res.on("end", () => {
    ...
    if (validWords.size < 100) {
        console.log("Download failed or format changed.");
        process.exit(1);
    }
    
    // Ensure the words the user tried are included
